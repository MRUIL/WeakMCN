...
# 定义要遍历的目录
#BASE_DIR="./logs/refcoco+_weakmcn/weakmcn/seed_123456"

BASE_DIR="./logs/refcocog_weakmcn/weakmcn/seed_123456"
...
for dir in "$BASE_DIR"/*; do
    if [ -d "$dir" ]; then  # Check if it is a directory
        ...
        if [ -d "$checkpoint_dir" ] && [ -f "$checkpoint" ]; then
            ...
            # Run srun command
            python test.py --config config/refcocog_tuning.yaml --eval-weights "$checkpoint"
        fi
    fi
done
