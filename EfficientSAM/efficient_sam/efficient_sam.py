...
class EfficientSam(nn.Module):
    ...
    def predict_masks(
        ...
        rescaled_batched_points = rescaled_batched_points.to(torch.float32)
    ...
    def get_rescaled_pts(self, batched_points: torch.Tensor, input_h: int, input_w: int):
        # batched_points = batched_points.float()
        # mask1 = batched_points[..., 0] >= 0
        # mask2 = (batched_points[..., 0] * self.image_encoder.img_size / input_w).to(torch.float64)
        # a = float(-1.0)
        return torch.stack(
            [
                torch.where(
                    batched_points[..., 0] >= 0,
                    (batched_points[..., 0] * self.image_encoder.img_size / input_w).to(torch.float64),
                    -1.0,  # 使用 -1.0 而不是 -1
                ),
                torch.where(
                    batched_points[..., 1] >= 0,
                    (batched_points[..., 1] * self.image_encoder.img_size / input_h).to(torch.float64),
                    -1.0,  # 使用 -1.0 而不是 -1
                ),
            ],
            dim=-1,
